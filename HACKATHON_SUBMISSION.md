...
The Web3 layer is implemented as a deployable Solidity suite under `contracts/` (FZONE, wearables, arena settlement, badges, guilds, referrals, VRF, tournaments, a DAO, and the Friendzone social suite: reputation, tipping, prediction markets, social staking, soulbound badges, chat, quests, leaderboards, multi-tier invites, and DAO engagement). See `contracts/SOCIAL.md`. A wallet-ready client boundary lives in `lib/web3`. The Expo demo still does not require a live chain for judging: Home and Settings connect MetaMask when an injected provider or the MetaMask mobile app is available, then fall back to the local demo wallet. The judge walkthrough CONNECT control stays on the instant demo wallet. Live `TugOfWarArena` create/join is available from Settings only after a live wallet is connected and `EXPO_PUBLIC_GAME_CONTRACT` or `lib/web3/addresses.ts` points at a deployed contract. The social layer models nearby Friendzone contacts and party handoff without pretending to be a live multiplayer service. The Crew lounge now labels this boundary as a Decentraland Bridge local demo, offers a resilient Plaza world handoff, and keeps event, parcel, party, and reward context visible even when external services are unavailable. This makes the prototype reliable for judging and leaves a clear boundary for connecting approved wallet, presence, and contract services later.
...
Describe the reward flow as **MetaMask-ready with an offline demo fallback**. Connecting MetaMask, using the judge walkthrough demo wallet, equipping the Friendzone Plaza Band, and saving a winning receipt do not mint by themselves. On-chain match create/join is available from Settings only after a live wallet is connected and a deployed `TugOfWarArena` address is configured. No live transaction is required for the judged offline walkthrough.
