...
import { ICON_BUTTON_SIZE } from "@/lib/mobile";
import { RopeTrack } from "@/components/arena/RopeTrack";
import { Swipeable } from "@/components/gestures";
import { PullControl } from "@/components/mobile";
import { createPullGate } from "@/lib/mobile-ux";
import { AnimatedPowerBar, AnimatedTimerText, ArenaAnimationLayer, SafeGameScreen, VictoryAnimation } from "@/components/game";
import { SafeReanimatedView, SafeSkiaCanvas } from "@/components/graphics";
import { ConfettiCannon } from "@/components/effects";
import { HapticService } from "@/lib/haptics";
import { GameClient } from "@/lib/game-client";
import { nextCountdown, type SwipeDirection } from "@/lib/animations";
import { PracticeDummy } from "@/components/arena/venue";
import { BalanceDisplay } from "@/components/wallet/BalanceDisplay";
import { ConnectWalletButton } from "@/components/wallet/ConnectWalletButton";
import { NetworkSwitcher } from "@/components/wallet/NetworkSwitcher";
import { OnChainMatchPanel } from "@/components/wallet/OnChainMatchPanel";
...
type Screen = "home" | "lobby" | "arena" | "results" | "leaderboard" | "settings" | "social" | "world" | "blockchain" | "governance";
type Team = "sun" | "moon";
...
export default function HomeScreen() {
  ...
  const hybridWorld = useHybridWorld();
  const walletAddress = account ? formatAddress(account) : "";
  ...
  const pullGateRef = useRef(createPullGate());
  const handledInviteRef = useRef<string | null>(null);
  const multiplayerRecordedRef = useRef(false);
  ...
  useEffect(() => {
    ...
    if (screen === "results" && !multiplayerRecordedRef.current) {
      multiplayerRecordedRef.current = true;
      void (async () => {
        await recordFinishedMatch({ won: playerWon, team, taps, duration: Math.max(0, 30 - time), rope: pull });
        const playerId = await getPlayerId();
        await LeaderboardService.getInstance().updatePlayerStats(playerId, playerWon, Math.max(5, taps));
      })();
    }
  }, [screen, playerWon, taps, team, time, pull]);
  ...
  const openPlazaWorld = async () => {
    ...
    const plazaUrl = resolveDecentralandWorldUrl();
    try {
  ...
  const connectWallet = async (mode: "auto" | "demo" = "auto") => {
    fireHaptic("medium");
    if (walletConnected) {
      await disconnect();
      setJudgeWalkthroughFeedback("Wallet disconnected · offline play remains available");
      announce("Wallet disconnected. Offline play remains available.");
      return;
    }
    try {
  ...
  const handleCountdownComplete = () => {
    setCountdown((current) => nextCountdown(current ?? -1));
  };
  ...
  const startMatch = () => {
    ...
    multiplayerRecordedRef.current = false;
    setCountdown(reduceMotion ? null : 3);
    setScreen("arena");
  ...
  useEffect(() => {
    if (screen !== "arena" || countdown !== null) return;
  ...
  const tapRope = () => {
    ...
    if (!pullGateRef.current.tryConsume()) return;
    GameClient.sendTap();
    ...
    setPull(nextPull);
    pullRef.current = nextPull;
    reportLocalPull("tap", { amount: swing, taps: nextTaps });
  ...
  const activateSurge = () => {
    ...
    setPull((current) => {
      ...
      pullRef.current = nextPull;
      reportLocalPull("swipe", { amount: 8, taps });
      return nextPull;
    });
  ...
  const handleArenaSwipe = (direction: SwipeDirection) => {
    if (direction === "up") activateSurge();
  };
  ...
  const header = (eyebrow: string, title: string, back = false) => (
    <View style={styles.header}>
      ...
      {back ? (
        <Pressable accessibilityRole="button" accessibilityLabel="Back to home" onPress={() => setScreen("home")} style={({ pressed }) => [styles.iconButton, pressed && styles.pressed]}>
  ...
  const nav = (
    <View style={styles.nav}>
      {[{ key: "home" as Screen, icon: "home", label: "Home" }, { key: "social" as Screen, icon: "groups", label: "Crew" }, { key: "world" as Screen, icon: "public", label: "World" }, { key: "settings" as Screen, icon: "tune", label: "Settings" }].map((item) => (
        <Pressable key={item.key} accessibilityRole="tab" accessibilityLabel={`Open ${item.label}`} accessibilityHint={`Navigates to the ${item.label} section`} accessibilityState={{ selected: screen === item.key }} onPress={() => setScreen(item.key)} style={({ pressed }) => [styles.navItem, screen === item.key && styles.navItemActive, pressed && styles.pressed]}>
  ...
  const runFullJudgeWalkthrough = () => {
    ...
    const [, reconnectStep, playStep] = getJudgeFullWalkthroughSequence();
  ...
  if (screen === "world") {
    return (
      <ScreenContainer scroll footer={nav} className="px-5">
        {header("FRIENDZONE WORLD", "3D Social Arena", true)}
        <SurfaceErrorBoundary surface="World Companion" onReturnToCrew={() => setScreen("social")}>
          <WorldCompanionScreen
            onEnterWorld={openPlazaWorld}
            onOpenBlockchain={() => setScreen("blockchain")}
            onOpenGovernance={() => setScreen("governance")}
          />
  ...
  if (screen === "blockchain") {
    return (
      <ScreenContainer scroll footer={nav} className="px-5">
        {header("WEB3", "Wallet and proofs", true)}
        <SurfaceErrorBoundary surface="Blockchain" onReturnToCrew={() => setScreen("social")}>
          <BlockchainScreen
            address={account}
            connected={walletConnected}
            demo={!walletConnected || account?.startsWith("0x7A3F") || account?.toLowerCase().includes("dead")}
            onConnect={() => void connect()}
            onContinueDemo={() => void disconnect()}
          />
        </SurfaceErrorBoundary>
      </ScreenContainer>
    );
  ...
  if (screen === "governance") {
    return (
      <ScreenContainer scroll footer={nav} className="px-5">
        {header("GOVERNANCE", "Official DAO", true)}
        <SurfaceErrorBoundary surface="Governance" onReturnToCrew={() => setScreen("social")}>
          <GovernanceScreen />
        </SurfaceErrorBoundary>
      </ScreenContainer>
    );
  ...
  if (screen === "lobby") {
    return <ScreenContainer scroll edges={["top", "bottom", "left", "right"]} containerClassName="bg-[#11142B]" className="px-5">
      ...
      <View style={styles.teamGrid}>
        {(["sun", "moon"] as Team[]).map((candidate) => {
          ...
          return <Pressable key={candidate} accessibilityRole="button" accessibilityLabel={candidate === "sun" ? "Select Sun Crew" : "Select Moon Crew"} accessibilityState={{ selected }} onPress={() => { setTeam(candidate); fireHaptic("light"); }} style={({ pressed }) => [styles.teamCard, { borderColor: selected ? color : "#3A407A", backgroundColor: selected ? `${color}1A` : C.midnight }, pressed && styles.pressed]}>
      ...
      <PracticeDummy teamColor={team === "sun" ? C.coral : C.cyan} reduceMotion={reduceMotion} />
  ...
  if (screen === "arena") {
    ...
    return <SafeGameScreen><ScreenContainer edges={["top", "bottom", "left", "right"]} containerClassName="bg-[#11142B]" className="px-5" footer={<Swipeable onSwipe={handleArenaSwipe} enabled={arenaActionsEnabled} threshold={36}><View style={styles.arenaActions}><PullControl onPull={tapRope} onLongPress={surgeReady && arenaActionsEnabled ? activateSurge : undefined} disabled={!arenaActionsEnabled} color={teamColor} reduceMotion={reduceMotion} /><Pressable accessibilityLabel="Use power surge" accessibilityHint={surgeReady && arenaActionsEnabled ? "Activates your charged Power Surge" : "Power Surge is unavailable until seven taps are charged and the match is active"} accessibilityState={{ disabled: !surgeReady || !arenaActionsEnabled }} onPress={activateSurge} disabled={!surgeReady || !arenaActionsEnabled} style={({ pressed }) => [styles.surgeButton, { borderColor: surgeReady && arenaActionsEnabled ? C.gold : "#3A407A", opacity: surgeReady && arenaActionsEnabled ? 1 : 0.45 }, pressed && styles.pressed]}><MaterialIcons name="bolt" size={25} color={C.gold} /><Text style={styles.surgeText}>{surgeActive ? "SURGING" : "SURGE"}</Text><Text style={styles.surgeSubtext}>{surgeReady && arenaActionsEnabled ? "ready" : "7 streak"}</Text></Pressable></View></Swipeable>} overlay={<>{showTutorial && <View accessibilityRole="summary" accessibilityViewIsModal accessibilityLiveRegion="polite" accessibilityLabel={`First match onboarding, step ${tutorialStep} of 3`} style={styles.tutorialBackdrop}><Animated.View style={[styles.tutorialCard, { opacity: tutorialOpacity, transform: [{ scale: tutorialScale }] }]}><View style={styles.tutorialIcon}><MaterialIcons name={tutorialStep === 1 ? "touch-app" : tutorialStep === 2 ? "groups" : "bolt"} size={30} color={C.ink} /></View><Text style={styles.tutorialKicker}>FIRST MATCH / 0{tutorialStep}</Text><Text style={styles.tutorialTitle}>{tutorialStep === 1 ? "Find your rhythm." : tutorialStep === 2 ? "Pull with your crew." : "Charge the surge."}</Text><Text style={styles.bodyCenter}>{tutorialStep === 1 ? "Tap Pull rapidly to move the rope." : tutorialStep === 2 ? "Your chosen crew sets the banner, color, and social context for the round." : "Hit seven taps in a row to charge Power Surge, then spend it to swing the rope."}</Text><View accessibilityRole="progressbar" accessibilityLabel="First match onboarding progress" accessibilityValue={{ min: 1, max: 3, now: tutorialStep }} style={styles.tutorialSteps}>{([1, 2, 3] as const).map((step) => <View key={step} style={[styles.tutorialStep, step <= tutorialStep && styles.tutorialStepActive]} />)}<Text style={styles.tutorialStepLabel}>{tutorialStep} of 3 · {tutorialStep === 1 ? "Learn the pull" : tutorialStep === 2 ? "Choose your crew" : "Use Power Surge"}</Text></View><View style={styles.tutorialActions}><Pressable accessibilityRole="button" accessibilityLabel={tutorialStep === 3 ? "Start the match" : "Show the next onboarding tip"} accessibilityHint={tutorialStep === 3 ? "Closes onboarding and starts your first pull" : "Moves to the next first-match tip"} accessibilityState={{ disabled: tutorialStepLocked }} onPress={advanceTutorial} disabled={tutorialStepLocked} style={({ pressed }) => [styles.primaryButton, styles.tutorialNext, tutorialStepLocked && { opacity: 0.65 }, pressed && styles.pressed]}><Text style={styles.primaryButtonText}>{tutorialStep === 3 ? "START MATCH" : "NEXT TIP"}</Text><MaterialIcons name="arrow-forward" size={22} color={C.ink} /></Pressable><Pressable accessibilityRole="button" accessibilityLabel="Skip first-match onboarding" accessibilityHint="Closes onboarding and keeps offline play available" onPress={dismissTutorial} style={({ pressed }) => [styles.tutorialSkip, pressed && styles.pressed]}><Text style={styles.tutorialSkipText}>SKIP FOR NOW</Text></Pressable></View></Animated.View></View>}{tutorialFeedback && <Animated.View accessibilityRole="alert" accessibilityLabel={tutorialFeedback} accessibilityLiveRegion="polite" style={[styles.judgeSettingsToastRow, { opacity: tutorialFeedbackOpacity, transform: [{ scale: tutorialFeedbackScale }] }]}><View style={styles.judgeSettingsToastContent}><MaterialIcons name="check-circle" size={16} color={C.mint} /><Text style={[styles.activityMeta, { color: C.mint }]}>{tutorialFeedback}</Text><Pressable accessibilityRole="button" accessibilityLabel="Replay first-match onboarding" accessibilityHint="Starts the three-step tutorial again without clearing saved progress" onPress={replayTutorial} style={({ pressed }) => [styles.judgeSettingsToastAction, pressed && styles.pressed]}><Text style={styles.judgeSettingsToastActionText}>REPLAY</Text><MaterialIcons name="replay" size={15} color={C.gold} /></Pressable></View></Animated.View>}<SafeReanimatedView><ArenaAnimationLayer countdown={countdown} onCountdownComplete={handleCountdownComplete} combo={streak} reduceMotion={reduceMotion} /></SafeReanimatedView></>}>
      ...
      <AnimatedPowerBar team={team === "sun" ? "red" : "blue"} power={powerLeft} maxPower={100} glow={surgeActive || powerLeft > 70} reduceMotion={reduceMotion} showCenterMarker style={{ marginTop: 14 }} accessibilityLabel={powerTrackAccessibilityPresentation.label} accessibilityHint={powerTrackAccessibilityPresentation.hint} />
      <View style={styles.arenaPanel}><SafeSkiaCanvas><RopeTrack pull={pull} teamColor={teamColor} opponentColor={opponentColor} reduceMotion={reduceMotion} crewScore={arenaScoreAccessibilityPresentation.crew.now} opponentScore={arenaScoreAccessibilityPresentation.opponent.now} crewLabel={arenaScoreAccessibilityPresentation.crew.label} opponentLabel={arenaScoreAccessibilityPresentation.opponent.label} /></SafeSkiaCanvas><Text accessibilityRole="summary" accessibilityLiveRegion="polite" accessibilityLabel={firstPullCoachingPresentation.accessibilityLabel} style={styles.arenaHint}>{firstPullCoachingPresentation.label}</Text></View>
    ...
    </ScreenContainer></SafeGameScreen>;
  }

  if (screen === "results") {
    return <SafeGameScreen><ScreenContainer scroll edges={["top", "bottom", "left", "right"]} containerClassName="bg-[#11142B]" className="px-5">
      <ConfettiCannon active={playerWon && !reduceMotion} reduceMotion={reduceMotion} />
      <VictoryAnimation isWin={playerWon} reduceMotion={reduceMotion} showParticles={false} />
      ...
      <Pressable accessibilityRole="button" accessibilityLabel="Return to the arena home screen" onPress={() => setScreen("home")} style={({ pressed }) => [styles.secondaryButton, pressed && styles.pressed]}><Text style={styles.secondaryButtonText}>BACK TO HOME</Text></Pressable>
