...
async function getCatalyst(chainId = schemas_1.ChainId.ETHEREUM_MAINNET, target, targetContent) {
    ...
    if (targetContent) {
        return { client: (0, dcl_catalyst_client_1.createContentClient)({ url: targetContent, fetcher: createFetchComponent() }), url: targetContent };
    }
    let catalystClient;
    if (chainId === schemas_1.ChainId.ETHEREUM_SEPOLIA) {
        catalystClient = (0, dcl_catalyst_client_1.createCatalystClient)({ url: 'peer.decentraland.zone', fetcher: createFetchComponent() });
    }
    else {
        const catalysts = (0, contracts_snapshots_1.getCatalystServersFromCache)('mainnet');
        for (const catalyst of catalysts) {
            try {
                const client = (0, dcl_catalyst_client_1.createCatalystClient)({ url: catalyst.address, fetcher: createFetchComponent() });
...
async function fetchWorldPermissions(worldName, targetContent) {
    ...
    const response = await fetch(url);
    if (!response.ok) {
        await (0, fetch_1.drainResponse)(response);
