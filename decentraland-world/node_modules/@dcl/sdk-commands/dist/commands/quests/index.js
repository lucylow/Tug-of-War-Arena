...
async function main(options) {
    ...
    if (options.args['--target']) {
        if (!new RegExp(utils_2.urlRegex).test(options.args['--target']) && !options.args['--target'].includes('localhost')) {
            throw new error_1.CliError('QUESTS_INVALID_TARGET', i18next_1.default.t('errors.quests.invalid_target'));
        }
    }
    ...
    if (path) {
        if (await fs.fileExists(path)) {
            ...
            try {
                ...
                throw new error_1.CliError('QUESTS_INVALID_JSON', i18next_1.default.t('errors.quests.invalid_json', { path }));
            }
            try {
                ...
                logger.error(error);
                throw new error_1.CliError('QUESTS_INVALID_QUEST_JSON', i18next_1.default.t('errors.quests.invalid_quest_json'));
            }
        }
        else {
            throw new error_1.CliError('QUESTS_FILE_DOES_NOT_EXIST', i18next_1.default.t('errors.quests.file_does_not_exist'));
        }
    }
    else {
        quest = await (0, utils_2.createQuestByPrompting)({ logger });
        if (!quest) {
            throw new error_1.CliError('QUESTS_CREATION_CANCELLED', i18next_1.default.t('errors.quests.creation_cancelled'));
        }
    }
...
async function executeListSubcommand(components, linkerOpts, baseURL, address) {
    ...
    if (!(0, eth_connect_1.isAddress)(address)) {
        throw new error_1.CliError('QUESTS_INVALID_ADDRESS', i18next_1.default.t('errors.quests.invalid_address'));
    }
...
async function executeActivateSubcommand(components, linkerOpts, baseURL, questId) {
    ...
    if (!isUuid(questId)) {
        throw new error_1.CliError('QUESTS_INVALID_UUID', i18next_1.default.t('errors.quests.invalid_uuid'));
    }
    await (0, utils_2.executeSubcommand)(components, linkerOpts, { url: activateQuest, method: 'PUT', metadata: {}, actionType: 'activate', extraData: { questId } }, async (authchainHeaders) => {
        ...
        if (res.status === 202) {
            await (0, fetch_1.drainResponse)(res);
