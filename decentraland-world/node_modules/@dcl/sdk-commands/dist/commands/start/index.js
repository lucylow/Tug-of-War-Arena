...
const open_1 = __importDefault(require("../../logic/open"));
        ...
        async main({ components, startComponents }) {
            ...
            if (withDataLayer) {
                try {
                    ...
                    dataLayer = await (0, rpc_1.createDataLayer)(components, projectWorkingDir);
                }
                catch (e) {
            ...
            if (watch) {
                for (const project of workspace.projects) {
                    await (0, file_watch_notifier_1.wireFileWatcherToWebSockets)(components, project.workingDirectory, project.kind);
                }
            }
            ...
            const bevyUrl = `https://decentraland.org/bevy-web/?preview=true&realm=${new URL(sortedURLs[0]).origin}&position=${baseCoords.x},${baseCoords.y}`;
            if (bevyWeb) {
                components.logger.log('Available on:\n');
                components.logger.log(`    ${bevyUrl}`);
                (0, beautiful_logs_1.printWarning)(components.logger, 'Chromium-based browsers require permission for websites to reach localhost (Local Network Access).\n' +
            ...
            if (bevyWeb && openBrowser && !skipClient && sortedURLs.length) {
                try {
                    await (0, open_1.default)(bevyUrl);
                }
                catch (_) {
