...
export function isUserRejected(error: unknown): boolean {
  ...
  return /user rejected|rejected the request|denied transaction|user denied|transaction was rejected|connection was rejected/i.test(
...
export function isPendingRequest(error: unknown): boolean {
  ...
  return /already pending|request already pending|already processing/i.test(readErrorMessage(error));
}
...
export function isConfigurationError(error: unknown): boolean {
  ...
  return /not configured|wallet not connected|connect metamask/i.test(message);
}
...
export function formatWalletError(error: unknown): string {
  if (isUserRejected(error)) return "Connection was rejected in MetaMask.";
  if (isPendingRequest(error)) return "A MetaMask request is already pending.";
  const code = getWalletErrorCode(error);
  if (code === 4902) return "This network is not available in the wallet yet.";
  if (code === -32603) return "MetaMask could not complete that request. Try again.";
  const message = readErrorMessage(error);
  if (/metamask is not available|no wallet provider/i.test(message)) {
