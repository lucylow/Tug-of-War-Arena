  ...
  try {
    if (kind === 'rain') {
      ...
      ParticleSystem.create(entity, {
        ...
        blendMode: ParticleBlend.ALPHA,
      })
    }
  } catch (error) {
    SceneErrorHandler.getInstance().recordFault('Weather particles unavailable', error)
...
export function stopWeather() {
  if (weatherRoot && ParticleSystem.has(weatherRoot)) {
    ParticleSystem.getMutable(weatherRoot).playbackState = ParticlePlayback.STOPPED
  }
}
