...
export const normalizeMessage = (message: Message) => {
  ...
  const [firstContentPart] = contentParts;

  // If there's only text content, collapse to a single string for compatibility
  if (contentParts.length === 1 && firstContentPart?.type === "text") {
    return {
      role,
      name,
      content: firstContentPart.text,
    };
  }
...
export const normalizeToolChoice = (
  ...
  if (toolChoice === "required") {
    ...
    if (!firstTool) {
      throw new Error("tool_choice 'required' could not resolve a configured tool");
    }
